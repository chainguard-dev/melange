...
func (c Context) GetWolfiPackages(ctx context.Context) (map[string]bool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed getting URI %s: %w", PackageIndex, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("non ok http response for URI %s code: %v", PackageIndex, resp.StatusCode)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to untar wolfi index: %w", err)
	}
...
func parseIndex(indexFile string) (map[string]bool, error) {
	...
	if err := sc.Err(); err != nil {
		return nil, fmt.Errorf("failed to scan index file %s: %w", indexFile, err)
	}
