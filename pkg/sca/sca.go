...
// versionedDepsEnabled reports whether versioned "depends:" should be
// generated for the package being built.  This governs both the shared
// libraries it links against and the pkg-config packages its .pc files
// require: they are the same kind of build-time coupling, and they are
// enabled and disabled together.
//
// They are generated by default; a package that does not want them
// opts out with the "no-versioned-shlib-deps" option.
func versionedDepsEnabled(opts config.PackageOption) bool {
	return !opts.NoVersionedShlibDeps
}

// providerQuery describes the "provides:" that a dependency is
// resolved through.
//
// resolve is what PkgResolver is asked for, and versioned is the
// versioned "provides:" the winning package must publish before we are
// willing to constrain a "depends:" on its version.  Shared libraries
// use two namespaces, so the two differ ("so:libz.so.1" resolves,
// "so-ver:libz.so.1" versions); pkg-config packages have only one, and
// a single "pc:openssl=<pkgver>" does both.
...
// package that provides the shared library shlib.
func determineShlibVersion(ctx context.Context, hdl SCAHandle, shlib string) (string, error) {
	return determineProviderVersion(ctx, hdl, providerQuery{
		resolve:   "so:" + shlib,
		versioned: "so-ver:" + shlib,
	})
}

// determinePkgConfigVersion tries to determine the exact version of the
// package that provides the pkg-config package pcName.
func determinePkgConfigVersion(ctx context.Context, hdl SCAHandle, pcName string) (string, error) {
	return determineProviderVersion(ctx, hdl, providerQuery{
		resolve:   "pc:" + pcName,
		versioned: "pc:" + pcName,
	})
...
// determineProviderVersion tries to determine the exact version of the
// package that provides q.  It does that by:
//
// - Asking PkgResolver to calculate the list of packages that provide q.resolve.
//
...
//     provided artifact whose version is equal or greater than the one
//     the build requires.
//
//   - Verifying if the matched package has a versioned "provides:" for
//     q.versioned.
//
// If all steps succeed, the package version is returned.
//
// If there is a match but the package doesn't currently offer a
// versioned "provides:", then an empty version is returned.  We can't
// do versioned "depends:" unless there is a matching "provides:".
//
// If no match is found, then we assume that the artifact is vendored by
// the package being built.  In this case, we return an empty version
// string.
func determineProviderVersion(ctx context.Context, hdl SCAHandle, q providerQuery) (string, error) {
	log := clog.FromContext(ctx)

	if !versionedDepsEnabled(hdl.Options()) {
		return "", nil
	}
	...
	if pkgResolver == nil {
		...
		// version.
		log.Debugf("Package resolver is nil; returning an empty version")
		return "", nil
	}

	// Obtain the list of packages that provide the artifact.
	candidates, err := pkgResolver.ResolvePackage(q.resolve, map[*apk.RepositoryPackage]string{})
	if err != nil {
		if strings.Contains(err.Error(), "nothing provides") {
			// We can reach here if one of the
			// (sub)packages we're building provides a new
			// version of the artifact.  Since this new
			// (sub)package isn't present in the current
			// APKINDEX, ResolvePackage() won't be able to
			// find it.  However, we can return the
			// current version being built in this case.
	...
	// ResolvePackage will return *all* packages that provide the
	// artifact, including those that have been obsoleted.  For
	// that reason, we need sort the list of candidates by build
	// time, making sure that we're always considering the newest
	// ones.
	...
	for _, candidate := range candidates {
		log.Debugf("Verifying if package %s-%s (which provides %s) is the one used to build the package", candidate.PackageName(), candidate.Version, q.resolve)
		...
		if installedPackageVersionString == "@CURRENT@" {
			...
			if apk.CompareVersions(candidateVersion, installedPackageVersion) < 0 {
				...
				// artifact that is versioned as older
				// than the one we have in our build
				// environment.
				return "", fmt.Errorf("could not find suitable package providing %s>=%s (found %s=%s instead)", q.versioned, installedPackageVersionString, q.versioned, candidate.Version)
			}

			// Check if the package actually publishes a
			// versioned "provides:", otherwise we can't
			// depend on it.
			if slices.ContainsFunc(candidate.Provides, func(provides string) bool {
				...
				if providedArtifact != q.versioned {
					return false
				}
			...
			// The package (still) doesn't publish a
			// versioned "provides:".  That's ok; just
			// return an empty version for now.
			return "", nil
		}
	}
	...
	//   dealing with a vendored artifact that isn't provided by
	//   any of our packages.  Just return an empty version string.
	//
	// - There is a list of candidates, but none of them provides
	//   the version of the artifact we're interested in.  This has
	//   been seen to happen when the package being built uses a
	//   vendored library that *is* provided by one of our
	...
	if err := fs.WalkDir(fsys, ".", func(path string, d fs.DirEntry, err error) error {
		...
		if isInDir(path, []string{"usr/local/lib/pkgconfig/", "usr/local/share/pkgconfig/", "usr/lib/pkgconfig/", "usr/lib64/pkgconfig/", "usr/share/pkgconfig/"}) {
			...
			if generateRuntimePkgConfigDeps {
				requires := []struct {
					kind string
					deps []pkgconfig.Dependency
				}{
					{"requires", pkg.Requires},
					{"requires private", pkg.RequiresPrivate},
					{"requires internal", pkg.RequiresInternal},
				}

				for _, req := range requires {
					for _, dep := range req.deps {
						log.Infof("  found pkg-config dependency (%s) %s for %s", req.kind, dep.Identifier, path)
						generated.Runtime = append(generated.Runtime, fmt.Sprintf("pc:%s", dep.Identifier))

						pcVer, err := determinePkgConfigVersion(ctx, hdl, dep.Identifier)
						if err != nil {
							return err
						}
						if pcVer != "" {
							generated.Runtime = append(generated.Runtime, fmt.Sprintf("pc:%s>=%s", dep.Identifier, pcVer))
						}
					}
				}
			}
