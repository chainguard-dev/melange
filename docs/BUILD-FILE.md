...
libraries this package links against, for example
`so-ver:libz.so.1>=1.3.1-r6`, and for the pkg-config packages its
`.pc` files require, for example `pc:openssl>=4.0.2-r1`, should not be
versioned.  In both cases the constraint names the version of the
package that satisfied the dependency at build time, so the two are
enabled and disabled together.

Versioned `depends` are generated by default; this option opts a
single package out of them.  It does not affect the versioned
`so-ver:` and `pc:` `provides` this package publishes for its own
shared libraries and `.pc` files.
