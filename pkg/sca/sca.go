	...
	for _, candidate := range candidates {
		...
		if installedPackageVersionString == "@CURRENT@" {
			...
			if apk.CompareVersions(candidateVersion, installedPackageVersion) < 0 {
				...
				// environment.
				return "", fmt.Errorf("could not find suitable package providing library so:%s>=%s (found so:%s=%s instead)", shlib, installedPackageVersionString, shlib, candidate.Version)
			}
