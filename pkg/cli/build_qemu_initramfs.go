...
func buildQemuInitramfs() *cobra.Command {
	var (
		outputPath   string
		archStr      string
		initPackage  string
...
Keys and modules are injected at runtime for each build.`,
  ...
  QEMU_BASE_INITRAMFS=./initramfs.cpio melange build --runner qemu ...`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return BuildQemuInitramfsCmd(cmd.Context(), outputPath, archStr, initPackage, repositories, packages, extraKeys)
		},
	}
...
// BuildQemuInitramfsCmd generates a base initramfs for the QEMU runner.
func BuildQemuInitramfsCmd(ctx context.Context, outputPath, archStr, initPackage string, repositories, packages, extraKeys []string) error {
